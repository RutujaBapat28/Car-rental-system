...
class car:
    def __init__(self,root):
        ...
        self.mainFrame.place(width=self.width/2.5, height=self.height-200,x=self.width/6-150,y=100)

        reserveBtn = tk.Button(self.mainFrame,command=self.reserve,text="Reserve Car",bg="sky blue", fg="black", bd=5, relief="raised",width=20,font=("Arial",30))
        reserveBtn.grid(padx=120, pady=60,row=0,column=0)
        
        returnBtn = tk.Button(self.mainFrame,command=self.returnFun,text="Return Car",bg="sky blue", fg="black", bd=5, relief="raised",width=20, font=("Arial",30))
        returnBtn.grid(padx=40, pady=60,row=1,column=0)
        
        closeBtn = tk.Button(self.mainFrame,command=self.closeWindow,text="Close Window",bg="sky blue", fg="black", bd=5, relief="raised",width=20, font=("Arial",30))
        closeBtn.grid(padx=40, pady=60,row=3,column=0)
        
        feedbBtn = tk.Button(self.mainFrame,command=self.feedback,text="FeedBack",bg="sky blue", fg="black", bd=5, relief="raised",width=20, font=("Arial",30))
        feedbBtn.grid(padx=40, pady=60,row=2,column=0)
        
        self.showFrame= tk.Frame(self.root,bg="light gray",bd=4,relief="groove")
        self.showFrame.place(width=self.width/3,height=self.height-200,x=self.width/2+50,y=100)
        
        lab= tk.Label(self.showFrame,text="Available Cars",bg="light gray",fg="red",font=("Arial",30,"bold"))
        lab.grid(row=0,column=1,padx=50,pady=20)
        
        self.list= tk.Listbox(self.showFrame,fg="white",font=("Arial",15,"bold"),bg="gray",bd=4,relief="sunken",width=53, height=20)
        self.list.grid(row=1,column=1,padx=10,pady=50,columnspan=2)
        self.retrieve()
        
    def db_conn(self):                         
        self.con = pymysql.connect(host="localhost", user="root", passwd="VRUSH@#2314", database="cars")
    ...
    def feedback(self):
        self.feedFrame= tk.Frame(self.root,bg="sky blue",bd=4,relief="groove")
        self.feedFrame.place(width=self.width/3,height=self.height-180,x=self.width/2+50,y=100)
        
        name = tk.Label(self.feedFrame,text="Name: ", bg="sky blue",fg="black", font=("Arial",15,"bold"))
        name.grid(row=0,column=0, padx=20, pady=50)
        self.name_entry= tk.Entry(self.feedFrame,bd=2, font=("Arial",15),width=30)
        self.name_entry.grid(row=0, column=1, padx=20, pady=50)
        
        email = tk.Label(self.feedFrame,text="Email: ", bg="sky blue",fg="black", font=("Arial",15,"bold"))
        email.grid(row=1, column=0, padx=20, pady=50)
        self.email1 = tk.Entry(self.feedFrame,bd=2, font=("Arial",15),width=30)
        self.email1.grid(row=1,column=1,padx=20, pady=50)

        text1 = tk.Label(self.feedFrame,text="Feedback: ", bg="sky blue",fg="black", font=("Arial",15,"bold"))
        text1.grid(row=2, column=0, padx=20, pady=50)
        self.text2 = tk.Text(self.feedFrame,bd=2, font=("Arial",15),width=30,height=3)
        self.text2.grid(row=2,column=1,padx=20, pady=20)
        
        okBtn= tk.Button(self.feedFrame, text="OK",command=self.feedfun, font=("Arial",20),width=20, bg="light gray", bd=2)
        okBtn.grid(row=3,column=0,padx=30, pady=60,columnspan=2)
    def feedfun(self):
        name = self.name_entry.get()
        email = self.email1.get()
        feedback = self.text2.get("1.0", tk.END).strip()
